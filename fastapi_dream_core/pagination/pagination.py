...
def _create_params(cls: Type[PageQuery], fields: Dict[str, Any]) -> Mapping[str, Any]:
...
class AbstractPage(GenericModel, Generic[T], ABC):
    __params_type__: ClassVar[Type[PageQuery]]

    @classmethod
    @abstractmethod
    def create(cls: Type[C], items: Sequence[T], total: int, params: PageQuery) -> C:
        pass
    ...
    def with_custom_options(cls: Type[TAbstractPage], **kwargs: Any) -> Type[TAbstractPage]:
        ...
        class CustomPage(*bases):  # type: ignore
            __params_type__: ClassVar[Type[PageQuery]] = custom_params
...
class Page(AbstractPage, Generic[T]):
    ...
    @classmethod
    def create(cls, items: Sequence[T], total: int, page_query: PageQuery):
        return cls(
            total=total,
            items=items,
